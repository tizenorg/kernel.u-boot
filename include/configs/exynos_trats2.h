...
#define CONFIG_EXTRA_ENV_SETTINGS \
	"checkpit=" \
		"if test ${pit_ver} = 13; then " \
			"setenv mmcbootpart 2;" \
			"setenv mmcrootpart 11;" \
			"setenv mmccsapart 4;" \
		"else " \
			"setenv mmcbootpart 2;" \
	...
	"mmcboot=run checkpit; set bootargs root=/dev/mmcblk0p${mmcrootpart} rw rootfstype=ext4 rootwait " \
		"${console} ${meminfo} ${debuginfo} " \
		"${bootmode} ${lcdinfo} ${muicpathinfo} ${lpjinfo} ${opts} csa=/dev/mmcblk0p${mmccsapart}; " \
