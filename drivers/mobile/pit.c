...
/* get pit version using pit that is written to the address */
static int check_pit_version(unsigned char *addr)
{
	pit_header_t *hd;
	partition_info_t *pi;
	int i;
	...
	pi = (partition_info_t *) (addr + sizeof(pit_header_t));
	...
	for (i = 0; i < hd->count; i++, pi++) {
		if (!strcmp(pi->name, "pit"))
			return name_to_version(pi->file_name);
	}
...
{
	if (!strcmp(name, "ums") || !strcmp(name, "user")) {
		/* UMS or user is the last partition */
...
static int check_pit_switch(unsigned char *addr)
{
	char *env;

	if ((env = getenv("pit_ver")) != NULL) {
		int pit_ver = simple_strtoul(env, NULL, 10);
		unsigned char *default_pit;

		if (pit_ver != check_pit_version(addr)) {
			switch(pit_ver) {
			case 8:
				default_pit = &default_pit08;
				pit_is_switched = 1;
				break;
			case 13:
				default_pit = &default_pit13;
				pit_is_switched = 1;
				break;
			}

			if (pit_is_switched) {
				memcpy(addr, default_pit, PIT_PKT_SIZE);
				mmc_cmd(MMC_WRITE, CONFIG_PIT_DEFAULT_ADDR,
					PIT_PKT_SIZE, (void *)addr);
				printf("Force update pit to %d\n", pit_ver);
				return pit_ver;
			}
...
static void do_format_parts(void)
{
	char buf[64];
	int i, percent, part_num = 0;
	char *fmt_parts[] = {PARTS_BOOT, PARTS_MODEM, PARTS_CSC,};
	int fmt_part_num = ARRAY_SIZE(fmt_parts);

	draw_format_logo();

	for (i = 0; i < fmt_part_num; i++) {
		int part_id = get_pitpart_id(fmt_parts[i]);

		if (pitparts[part_id].dev_type == PIT_DEVTYPE_MMC &&
		     (pitparts[part_id].filesys == PART_FS_TYPE_EXT4 ||
		      pitparts[part_id].filesys == PART_FS_TYPE_ENHANCED)) {
			int part_num = pit_adjust_id_on_mmc(part_id);

			sprintf(buf, "ext4format mmc 0:%x\n", part_num);
			run_command(buf, 0);

			percent = ((i + 1) * 100) / fmt_part_num;
		       ...
		       char *const argv[])
{
	...
	if (check_pit_integrity(addr)) {
		/* force write default pit */
		check_pit_switch(addr);
		return ret;
	}

	/* check that pit need to switch */
	check_pit_switch(addr);
