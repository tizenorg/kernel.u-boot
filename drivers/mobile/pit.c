...
int pit_adjust_id_on_mmc(int index)
{
	...
	debug("Pit id is adjusted by GPT (%d -> %d)\n",
	       index, boot_id);
 ...
 */
static void set_env_uuid_name(char *name)
{
	int i;
	char uuid[37];
	...
	uuid_bin_to_str((unsigned char *)u_name, uuid);
...
void check_pit(void)
{
	...
	/* Read PIT from MMC */
	ret = mmc_cmd(MMC_READ, CONFIG_PIT_DEFAULT_ADDR,
			CONFIG_PIT_DEFAULT_SIZE, (void *)pit);

	/* If error or no PIT found then write default PIT version 08 */
	if (ret || check_pit_integrity(pit)) {
		puts("Pit not found. Writing PIT version 08.\n");
		mmc_cmd(MMC_WRITE, CONFIG_PIT_DEFAULT_ADDR,
			PIT_PKT_SIZE, (void *)&default_pit08);

		sprintf(buf, "pit update %p", &default_pit08);
	} else {
		sprintf(buf, "pit update %p", pit);
	}

	run_command(buf, 0);

	pit_version = pit_get_version();
	printf("PIT version: %d\n", pit_version);

	sprintf(buf, "%d", pit_version);
	setenv("dfu_alt_pit_compatible", buf);
}
...
void pit_to_dfu_alt_info(void)
{
	...
	char *dfu_dev_type;
	char *dfu_part_boot;
	unsigned int dfu_dev_arg1, dfu_dev_arg2;
	...
	for (i = 0; i < hd->count; i++, pi++) {
		...
		if (pi->dev_type == PIT_DEVTYPE_MMC) {
			/* dfu entity: file_name mmc start count */
			dfu_file_name = pi->name;
			dfu_dev_type = "mmc";
			dfu_dev_arg1 = pi->blk_start;
			dfu_dev_arg2 = pi->blk_num;

			if (!strncmp(dfu_file_name, "s-boot", 6))
				dfu_part_boot = "mmcpart 1";
		} else {
			if (!strncmp(pi->name, "install-", 8) ||
			    !strncmp(pi->name, "fota-delta", 10))
				tmp_id = get_pitpart_id(PARTS_UMS);
			else
				tmp_id = get_pitpart_id(PARTS_BOOT);

			/* dfu entity: file_name ext4 mmc_dev part_num */
			dfu_file_name = pi->file_name;
			dfu_dev_type = "ext4";
			dfu_dev_arg1 = CONFIG_MMC_DEFAULT_DEV;
			dfu_dev_arg2 = pit_adjust_id_on_mmc(tmp_id);
		}

		/* Over write entity for UMS */
		if (!strncmp(pi->name, "ums", 3)) {
			dfu_dev_type = "part";
			dfu_dev_arg1 = CONFIG_MMC_DEFAULT_DEV;
			tmp_id = get_pitpart_id(PARTS_UMS);
			dfu_dev_arg2 = pit_adjust_id_on_mmc(tmp_id);
	...
	sprintf(dfu_alt_num, "%d", hd->count + add_alt_dtb);
