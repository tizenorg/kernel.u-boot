				 ...
				 ext2_inode_scan *ret_scan)
{
	...
	retval = ext2fs_get_memalign(scan->inode_buffer_blocks * fs->blocksize,
				     fs->blocksize, (char**)&scan->inode_buffer);
	scan->done_group = 0;
...
static errcode_t get_next_blocks(ext2_inode_scan scan)
{
	...
	if ((scan->scan_flags & EXT2_SF_BAD_INODE_BLK) ||
		...
		retval = io_channel_read_blk(scan->fs->io,
					     ...
					     scan->inode_buffer);
#endif
		int factor = scan->fs->blocksize/512;
		retval= ext2fs_devread(scan->current_block * factor , 0, (scan->fs->blocksize*num_blocks),scan->inode_buffer);
		retval=0; //uma hack
...
extern unsigned int my_journal_blocks;
void *my_buffer=NULL;
unsigned long my_block_nr;
				  ...
				  struct ext2_inode * inode, int bufsize)
{
	...
	static unsigned int prev_bufsize=0;
	static int FLAG=0;
...
//uma
		...
		{
			put_ext4(dev_desc, (uint64_t)(block_nr * fs->blocksize), fs->icache->buffer,(uint32_t)(fs->blocksize));			
		}		
		retval=0;
		offset = 0;
		ptr += clen;
