...
/* closefs.c */
extern errcode_t ext4fs_close(ext2_filsys fs);
...
//uma extern errcode_t ext2fs_get_memalign(unsigned long size,
...
//umaa
extern void put_ext4(block_dev_desc_t *dev_desc,uint64_t off, void *buf, uint32_t size);
...
{	
	...
	#if 0
	//uma 
	 if (retval = posix_memalign((void **) ptr, align, size)) {z
		if (retval == ENOMEM)
			return EXT2_ET_NO_MEMORY;
		return retval;
	}
